...
4. **asker_pdf_sent** — גוף סטטי (בלי משתני גוף); כפתור URL דינמי עם קידום קבוע זהה ל־`NEXT_PUBLIC_APP_URL` ומשתנה `{{1}}` = נתיב ושאילתה בלבד (למשל `api/questions/<uuid>/pdf/download?for=asker`). הקוד שולח `bodyParameters: []` ואת הסיומת בכפתור בלבד.
5. **cron_lobby_summary** — `{{1}}` = מספר משימות (מחרוזת); **כפתור URL דינמי**: סיומת ל־`/proofreader` דרך `/api/go`. (גרסאות ישנות עם `{{2}}` לקישור בגוף — לא בשימוש אחרי v3.)
...
4. **asker_pdf_sent_v2** — גוף ללא `{{1}}`; כפתור מסוג URL עם כתובת בסיס (למשל `https://www.adeyadamp.com/`) ומשתנה אחד בנתיב; טקסט הכפתור למשל `להורדת התשובה (PDF)`.

```text
שלום וברכה,

שמחים לעדכן כי צוות אסק מי פלוס השיב לפנייתך.
בלחיצה על הכפתור למטה ניתן להוריד את קובץ התשובה (PDF).

המידע בתשובה הינו כללי ואינו מהווה תחליף לייעוץ מקצועי אישי.
```

5. **cron_lobby_summary_v3** (CTA: כפתור URL דינמי — קישור בכפתור בלבד)

```text
שלום וברכה,
היום יש {{1}} משימה/ות ממתינות בלובי ההגהה.
לחץ/י על הכפתור למטה לכניסה.
יום נעים!
```
...
- **סיכום שאלות בפוש (אחראי טכני בלבד)** – `/api/cron/questions-digest-push` (שני+חמישי ב־10:00), סופר כמה שאלות חדשות נכנסו מאז הריצה הקודמת.

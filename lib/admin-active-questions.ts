...
const BASE_SELECT =
  "id, short_id, stage, title, content, created_at, updated_at, asker_email, asker_age, asker_gender, response_type, publication_consent, assigned_respondent_id, assigned_proofreader_id, response_text, proofreader_note, pdf_url, pdf_generated_at, proofreader_type_id";
...
const QA_SELECT =
  "id, question_id, topic_id, sub_topic_id, assigned_respondent_id, assigned_proofreader_id, stage, response_text, proofreader_note, pdf_url, pdf_generated_at, proofreader_type_id, created_at, updated_at, deleted_at";
...
function maxUpdatedAt(group: QuestionAnswerRaw[]): string | null {
  const times = group.map((r) => r.updated_at).filter(Boolean) as string[];
  if (times.length === 0) return null;
  return times.reduce((a, b) => (new Date(a) > new Date(b) ? a : b));
}

/** שאלות פעילות ללוח הבקרה — אותה לוגיקה לטבלה ולסרגל עיכובים (כולל שורות מ־question_answers). */
export async function getActiveQuestions(): Promise<QuestionRow[]> {
  ...
  const fromAnswer = (r: QuestionAnswerRaw): QuestionRow => {
    ...
    return {
      ...
      delay_source_updated_at: r.updated_at ?? null,
    };
  };
  ...
  for (const [, group] of byQuestionIdReady) {
    ...
    answerRows.push({
      ...
      delay_source_updated_at: maxUpdatedAt(group),
    });
  }

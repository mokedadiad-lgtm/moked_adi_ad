-- כשהמשיב מוגדר ב-question_answers, submit_respondent_response עדכן רק את שורת התשובה.
-- notifyLobbyNewQuestion בודקת את questions.stage — אם לא מעדכנים גם את questions, השלב נשאר with_respondent
-- ואז לא נשלחים מייל/וואטסאפ למגיהים.
...
COMMENT ON FUNCTION submit_respondent_response(uuid, uuid, text, uuid) IS 'משיב שולח תשובה להגהה; מסנכרן גם questions.stage כשהשורה ב-question_answers';
